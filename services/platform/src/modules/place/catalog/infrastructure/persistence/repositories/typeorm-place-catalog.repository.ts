...
import type { PlaceCatalogRepositoryPort } from '../../../application/ports/place-catalog-repository.port.js';
import type { FindNearbyPlacesQuery } from '../../../application/queries/find-nearby-places.query.js';
import type {
  SearchPlacesQuery,
  SearchPlacesResult,
} from '../../../application/queries/search-places.query.js';
...
import {
  ...
  PlaceDetailRow,
  PlaceSearchRow,
} from '../mappers/place-catalog.mapper.js';

@Injectable()
export class TypeormPlaceCatalogRepository implements PlaceCatalogRepositoryPort {
  constructor(
  ...
  async search(query: SearchPlacesQuery): Promise<SearchPlacesResult> {
    ...
    return {
      items: rows.map((row) => PlaceCatalogMapper.toListItem(row)),
      total,
      page: query.page,
      limit: query.limit,
    };
  ...
  async findById(placeId: string): Promise<PlaceCatalogDetailReadModel | null> {
    ...
    return PlaceCatalogMapper.toDetailItem(row);
  }
